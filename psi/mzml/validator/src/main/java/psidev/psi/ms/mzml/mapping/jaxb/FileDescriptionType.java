...
public class FileDescriptionType {
    ...
    private String elementName = "fileDescription";

    public FileDescriptionType() {}

    public FileDescriptionType(String xmlSnippet) {
        parseXml(xmlSnippet);
    }

    public FileDescriptionType(Element element) {
        create(element);
    }
    ...
    private void create(Node node) {
        ...
        if (ele != null) {
            // load attributes
            // load elements
            NodeList nl = ele.getElementsByTagName("fileContent");
            // we only expect one such element!
            if (nl.getLength() == 1) {
                fileContent = new ParamGroupType((Element) nl.item(0));
            } else {
                throw new IllegalStateException("Expected only one 'fileContent' element, but found: " + nl.getLength());
            }
            nl = null;
            nl = ele.getElementsByTagName("sourceFileList");
            // we only expect one such element!
            if (nl.getLength() == 1) {
                sourceFileList = new SourceFileListType((Element) nl.item(0));
            } else {
                throw new IllegalStateException("Expected only one 'sourceFileList' element, but found: " + nl.getLength());
            }
            nl = null;
            nl = ele.getElementsByTagName("contact");
            for ( int i = 0; i < nl.getLength(); i++ ) {
                this.getContact().add(new ContactType((Element) nl.item(i)));
            }
